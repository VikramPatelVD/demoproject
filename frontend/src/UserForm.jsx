...
function UserForm({ editingUser, onSave, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ background: '#f9f9f9', padding: '20px', borderRadius: '8px' }}>
      ...
      <button type="submit" style={{ padding: '10px 20px', background: '#1F4E79', color: 'white', border: 'none', borderRadius: '4px', cursor: 'pointer' }}>
        {editingUser ? 'Update User' : 'Register'}
      </button>

      {editingUser && (
        <button type="button" onClick={onCancel} style={{ marginLeft: '10px', padding: '10px 20px' }}>Cancel</button>
      )}
    </form>
  );
}
